...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <Link
      href={`/shop/${slug}`}
      className={`group block h-full ${!purchasable ? "pointer-events-none" : ""}`}
    >
      <div
        className={`bg-background rounded-premium overflow-hidden shadow-card hover:shadow-card-hover transition-all duration-300 h-full flex flex-col ${
          isOutOfStock ? "opacity-60 grayscale-[0.15]" : ""
        }`}
      >
        {/* Product Image */}
        <div className="relative aspect-[3/4] bg-gradient-to-br from-background-sand to-background-ivory overflow-hidden">
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-1.5">
            {/* Product badge (New / Best Seller / Featured) */}
            {badge && (
              <div className={`px-3 py-1 rounded-full ${badge.className}`}>
            ...
            {/* Low Stock badge */}
            {isLowStock && (
              <div className="px-3 py-1 rounded-full bg-amber-500">
            ...
            {/* Out of Stock badge */}
            {isOutOfStock && (
              <div className="px-3 py-1 rounded-full bg-gray-800">
